...
$(function(){
	...
	applySlickPlugin('[data-slider="slick"]', {
		nextArrow: '<button type="button" class="slick-next-custom"><i class="far fa-arrow-alt-circle-right"></i></button>',
		prevArrow: '<button type="button" class="slick-prev-custom"><i class="far fa-arrow-alt-circle-left"></i></button>'
	});

	// carousel slider with 3 item only display
	applySlickPlugin('.client_list_slick', {
	...
	applySlickPlugin('.latest_news_slick', {
		slidesToShow:2, slidesToScroll:1,
		appendArrows: $('.latest_news_text') ,
		nextArrow: '<button type="button" class="slick-next-custom"><i class="fas fa-arrow-circle-right"></i></button>',
		prevArrow: '<button type="button" class="slick-prev-custom"><i class="fas fa-arrow-circle-left"></i></button>',
		responsive: [
			{ breakpoint: 768, settings: { slidesToShow: 2, slidesToScroll: 1 } }
		]
	});
	...
	$doc.on('click', '.navbar-toggler', function(e){
		e && e.preventDefault();
		var target = $(this).data('target');
		var isOpened = $body.hasClass('nav-opened');

		if(!isOpened) {
			$('#'+target).addClass('open');
			$body.addClass('nav-opened').append('<div class="nav-drop"></div>');
		}else{
			$('#'+target).removeClass('open');
			$body.removeClass('nav-opened').find('.nav-drop').remove();
		}
			
	});

	// close drawer menu on mobile when click on damed overlay
	$doc.on('click', '.nav-drop', function(e){
		e && e.preventDefault();
		$('.head_controls').removeClass('open');
		$body.removeClass('nav-opened').find('.nav-drop').remove();
	});
